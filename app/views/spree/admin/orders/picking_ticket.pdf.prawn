...
repeat(:all) do
  ...
  grid([0,3], [0,4]).bounding_box do
    ...
    text Spree.t(:picking_ticket, scope: :print_invoice), align: :right, style: :bold, size: 18
    move_down 4
...
grid([1,0], [6,4]).bounding_box do
  ...
  repeat(lambda { |pg| pg == 1 }) do
    ...
    billing =  "#{bill_address.company rescue ''}"
    ...
    shipping = "#{ship_address.company rescue ''}"
  ...
  @order.line_items.each do |item|
    cat_and_subcat_array = item.variant.cat_and_subcat
    form_sku = %Q{#{item.variant.sku}\n#{cat_and_subcat_array.first} #{cat_and_subcat_array.last}}
    row = [
  ...
  table(data, header: true, position: :center, column_widths: [60, 150, 155, 40, 40,40,55]) do
